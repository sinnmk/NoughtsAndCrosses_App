...
describe("GameRules Component", () => {
    ...
    it("shows the game rules title", () => {
        const wrapper = mount(<GameRules/>)
        expect(wrapper.find('[data-id="game-title"]').text()).toContain("Noughts + Crosses")
    })
    it("shows the game rules sub title", () => {
        const wrapper = mount(<GameRules/>)
        expect(wrapper.find('[data-id="sub-title"]').text()).toContain("Game Rules")
    })
});